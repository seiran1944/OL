package MVCprojectOL.ViewOL.StorageUI 
{
	 ...
	 * @author brook
	 */
	public class WallExitBtn implements IfMenuConter
	{
		...
		private function btnChange(e:MouseEvent):void 
		{
			...
			{
				case "rollOver":
					(this._Layer == 0 ||this._Layer == 1 || this._Layer == 2)?e.target.gotoAndStop(2):e.target.gotoAndStop(4);
					if (!TweenMax.isTweening(e.target)) this.onCompleteShake(e.target , true , 4);
				break;
				case "rollOut":
					(this._Layer == 0 ||this._Layer == 1 || this._Layer == 2)?e.target.gotoAndStop(1):e.target.gotoAndStop(3);
				break;
				case "click":
				if (this._Layer != 3 && this._Layer != 4) this._funPublicfun("Exit");
					this._funSend(  StorageStrLib.RemoveALL );
				break; 
			}
		}
		...
		public function AddVaules(_vaules:*):void
		{
			...
			(this._Layer == 1 || this._Layer == 2)?MovieClip(this._conter.getChildByName("ExitBtn")).gotoAndStop(1):MovieClip(this._conter.getChildByName("ExitBtn")).gotoAndStop(3);
		}
