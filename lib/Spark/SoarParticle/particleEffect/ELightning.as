package Spark.SoarParticle.particleEffect {
	 ...
	 * @author Andrew
		...
		@reference Pierluigi Pesenti / 0.5 /  http://blog.oaxoa.com/
	 */
	
	public class ELightning extends EffectSP {
		
		public static const SMOOTH_COLOR:uint = 0x808080;
		...
		public var parentInstance:ELightning;
		...
		public function ELightning(pcolor:uint=0xffffff, pthickness:Number=2, pgeneration:uint=0,name:String="") {
			super(name);
			...
			alphaFadeType=ELightning.GENERATION;
			thicknessFadeType=ELightning.NONE;
		...
		private function init():void {
			...
			if(lifeSpan>0) startLifeTimer();//計時與否
		...
		{
			this.childrenLifeSpanMax = 1;
			this.childrenLifeSpanMin = .5;
			this.childrenMaxCount = 8;
			this.childrenAngleVariation = 140;
			//躍動範圍限定 (預設高度100 時 70/50 )
			this.maxLength = 270;
			this.maxLengthVary = 50;
			//
			this.childrenProbability = .1;
			this.childrenProbabilityDecay = .5;
			this.childrenDetachedEnd = true;
			this.steps = 50;
		}
		...
		override public function Start(times:Number=0):void 
		{
			...
			TimeDriver.AddEnterFrame(30, 0, this.update);
		...
		public function startLifeTimer():void {
			TimeDriver.AddDrive(lifeSpan * 1000, 1, this.onTimer);
		}
		private function onTimer():void {
			TimeDriver.RemoveDrive(this.onTimer);
			//trace("DriverOntimer");
			kill();
		}
		public function kill():void {
			...
			if(parentInstance!=null) {
				var count:uint=0;
				var par:ELightning = this.parent as ELightning;
		...
		public function killAllChildren():void {
			while(childrenArray.length>0) {
				var child:ELightning=childrenArray[0].instance;
				child.kill();
			}
		}
			...
			if(generation<childrenMaxGenerations && childrenArray.length<childrenMaxCount) {
				...
				if(targetChildSteps>=2) {
					for(var i:uint=0; i<n; i++) {
						...
						this.count++;
						var child:ELightning = new ELightning(color, thickness, generation + 1);
						//trace("i create a child lightning            "  +  this.count);
						//child.name = "Mname  "+this.name + "children" + String(this.count);
						...
						if(alphaFadeType==ELightning.GENERATION) child.alpha=1-(1/(childrenMaxGenerations+1))*child.generation;
						if(thicknessFadeType==ELightning.GENERATION) child.thickness=thickness-(thickness/(childrenMaxGenerations+1))*child.generation;
		...
		public function render():void {
			...
			for (var i:uint=0; i<steps; i++) {
				...
				if(alphaFadeType==ELightning.TIP_TO_END) {						
					relAlpha=absolutePosition*currentPosition;
				}
				if(thicknessFadeType==ELightning.TIP_TO_END) {						
					relThickness=thickness*(absolutePosition*currentPosition);
				}
				
				if(alphaFadeType==ELightning.TIP_TO_END || thicknessFadeType==ELightning.TIP_TO_END) {
		...
		public function killSurplus():void {
			while(childrenArray.length>childrenMaxCount) {
				var child:ELightning=childrenArray[childrenArray.length-1].instance;
				child.kill();
			}
		}
		...
		override protected function EndCountProcess():void
		{
			//particle leng = 0;
			TimeDriver.RemoveDrive(this.onTimer);
			TimeDriver.RemoveDrive(this.update);
			this.kill();
		}
		
		override public function Destroy():void 
		{
			if (TimeDriver.CheckRegister(this.onTimer)) TimeDriver.RemoveDrive(this.onTimer);
			if (TimeDriver.CheckRegister(this.update)) TimeDriver.RemoveDrive(this.update);
